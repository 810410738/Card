...
def edit(request):
    if request.method == 'GET':
        ...
        qq = request.POST['qq']
        is_exist = models.Cards.objects.filter(username=username, title=title, name=name, phone=phone, address=address,
                                               email=email,
                                               wechat=wechat, qq=qq)
        if is_exist:
            return HttpResponse("已经存在了")
        else:
            models.Cards(username=username, title=title, name=name, phone=phone, address=address, email=email,
                         wechat=wechat, qq=qq).save()
...
def test(request):
    if request.method == 'GET':
        return render(request, 'CardSystem/test.html')
    elif request.method == 'POST':
        username = request.session.get('login_user', "")
        title = request.POST['title']
        name = request.POST['name']
        ...
        is_exist = models.Cards.objects.filter(username=username, title=title, name=name, phone=phone, qq=qq)
        if is_exist:
            return HttpResponse("已经存在了")
        else:
            models.Cards(username=username, title=title, name=name, phone=phone, qq=qq, image0=image0).save()
            return HttpResponse("创建成功！")
